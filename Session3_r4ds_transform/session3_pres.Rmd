---
title: "Session 3: Transforming Data"
...
These materials are from the [R for Data Science Book: Transformation Chapter](https://r4ds.had.co.nz/transform.html), written by Hadley Wickham & Garrett Grolemund. I only made small changes. 
      
We are now going into the `tidyverse` realm.     
`tidyverse` is a universe of R packages that work well togehter and eases many painful steps of data exploration or cleaning in R.   


# Introduction

**It is rare that you get the data in exactly the right form you need.** Often you'll need to create some new variables or summaries, or maybe you just want to rename the variables or reorder the observations in order to make the data a little easier to work with. You'll learn how to do all that (and more!) in this chapter, which will teach you how to transform your data using the dplyr package and a new dataset on flights departing New York City in 2013.

## Prerequisites

In this session we're going to focus on how to use the dplyr package, another core member of the tidyverse. We'll illustrate the key ideas using data from the nycflights13 package, and use ggplot2 to help us understand the data. 
...
To install packages, simply execute the following **on your console** (don't add this to your R script.):
...
Take careful note of the conflicts message that's printed when you load the tidyverse. It tells you that dplyr package overwrites some functions in base R. If you want to use the base version of these functions after loading dplyr, you'll need to use their full names: `stats::filter()` and `stats::lag()`.
...
Multiple arguments to `filter()` are combined with "and": every expression must be true in order for a row to be included in the output. For other types of combinations, you'll need to use Boolean operators yourself: `&` is "and", `|` is "or", and `!` is "not". Figure below shows the complete set of Boolean operations.
