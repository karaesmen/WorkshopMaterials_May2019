...
# See the content in your wd
dir()

# Or even better, create a project!

#### Your turn ####

# 1. Load `crime.csv` using `read.table()` or `read.csv()` 
#    and assign it to variable `crime`.
...
sum(titanic$female[titanic$fate == "perished"])
...
pw <- ggplot(wages2, aes(x=height, y=earn)) +
...
# Plot the `wages2.csv` data like the following

pw <- ggplot(wages2, aes(x=age, y=earn, color=sex))
